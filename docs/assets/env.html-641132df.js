...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><h3 id="打包时开启gzip压缩" tabindex="-1"><a class="header-anchor" href="#打包时开启gzip压缩" aria-hidden="true">#</a> 打包时开启gzip压缩</h3><p>由于前端项目引入了其他第三方框架，打包发布首屏加载可能由于网络原因导致加载过慢的问题</p><h3 id="部署时带固定前缀" tabindex="-1"><a class="header-anchor" href="#部署时带固定前缀" aria-hidden="true">#</a> 部署时带固定前缀</h3><p>例如访问路径需要调整为：</p><div class="language-bash line-numbers-mode" data-ext="sh"><pre class="language-bash"><code>http://cdesign.fun/cola/
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><ol><li><p>修改前端打包配置</p><p>前端打包配置需要修改3个文件：</p><p>①. vue.config.js中添加public配置</p><p><img src="`+i+'" alt="login.png"></p><p>②. src/router/index.js调整路由模式为hash并设置base路径</p><p><img src="'+t+'" alt="login.png"></p><p>③. env.js调整静态文件和api的前缀路径（根据实际使用场景而定）</p><p><img src="'+l+`" alt="login.png"></p></li><li><p>修改nginx配置(参考)</p></li></ol><div class="language-text line-numbers-mode" data-ext="text"><pre class="language-text"><code>server {
   listen       80;
   server_name  cdesign.fun;
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div>`,37),p=[r];function c(o,u){return s(),a("div",null,p)}const m=n(d,[["render",c],["__file","env.html.vue"]]);export{m as default};
